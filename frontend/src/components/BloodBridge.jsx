...
const ORBIT_R    = 120;                   // donor orbit radius
const PATIENT_R  = 40;                    // patient circle radius (40px = 80px diam)
const DONOR_R    = 25;                    // donor circle radius (25px = 50px diam)
const RING_R     = 50;                    // completeness ring radius
...
/* edge point of a circle toward another point */
...
function CompletenessRing({ donors, targetSize, cx, cy }) {
  ...
  return (
    <g>
      {/* full background ring — dashed gray */}
      <circle
      ...
      {pct > 0 && (
        <path
          d={arcPath(cx, cy, RING_R, 0, filledEnd)}
          fill="none"
          stroke={C.green}
          strokeWidth={5}
          strokeLinecap="round"
...
function DonorNode({ slot, angle, onSelect, selected }) {
  ...
  return (
    ...
    >
      {/* pulse ring for awaiting */}
      {cfg.pulse && (
        <circle
          cx={pos.x} cy={pos.y} r={DONOR_R + 6}
          fill="none"
          stroke={C.amber}
          strokeWidth={2}
          opacity={0.35}
      ...
      {/* hover scale wrapper via CSS class */}
      <g
        className={!isOpen ? 'donor-node' : ''}
        style={{ transformOrigin: `${pos.x}px ${pos.y}px` }}
      >
        <circle
          cx={pos.x} cy={pos.y} r={DONOR_R}
          fill={cfg.fill}
          stroke={cfg.border}
          strokeWidth={2.5}
          strokeDasharray={cfg.solid ? 'none' : 'none'}
        />
        {/* text */}
        {isOpen ? (
          <text
...
function ConnectionLine({ slot, angle }) {
  ...
  const dashArr = cfg.solid ? 'none' : '4 3';
...
function Popover({ donor, svgRef, angle, onHeal, onClose }) {
  ...
  // position the popover card near the donor node using bounding rect
  const svgRect = svgRef.current.getBoundingClientRect();
  const svgW = svgRect.width;
  const svgH = svgRect.height;
  const scale = svgW / SVG_SIZE;

  const pos = polarToXY(angle, ORBIT_R);
  // pixel coords relative to svg container
  let px = pos.x * scale;
  let py = pos.y * scale;

  // push popover outward
  const normAngle = ((angle % 360) + 360) % 360;
  ...
  px = Math.max(0, Math.min(px + offsetX, svgW - 170));
  py = Math.max(0, Math.min(py + offsetY, svgH - 160));
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 10,
        boxShadow: '0 8px 24px rgba(10,37,64,0.13)',
        padding: '12px 14px',
        zIndex: 20,
        pointerEvents: 'auto',
      }}
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: C.navy, lineHeight: 1.3 }}>
          {donor.name}
        </div>
        <button
          onClick={onClose}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted, fontSize: 16, padding: 0, lineHeight: 1 }}
          aria-label="Close"
        >×</button>
      </div>
      {donor.blood_group && (
      ...
      {donor.status === 'lapsed' && (
        <button
          onClick={() => { onHeal && onHeal(donor); onClose(); }}
          style={{
            marginTop: 10,
            width: '100%',
            fontSize: 12,
...
export default function BloodBridge({
  ...
  // build slot array (always targetSize entries, null = open)
  const slots = Array.from({ length: targetSize }, (_, i) => donors[i] || null);

  // counts
  const counts = {};
  slots.forEach(s => {
    const key = s ? s.status : 'open';
  ...
  const selectedAngle   = selectedDonor
    ? (slots.indexOf(selectedDonor) / targetSize) * 360
    : 0;

  const handleSelect = useCallback(donor => {
    setSelectedId(prev => (prev === donor.id ? null : donor.id));
  }, []);
      ...
      >
        ...
        >
          <svg
            ...
            aria-label="Blood bridge radial map"
          >
            {/* ── Connection lines (below nodes) ── */}
            {slots.map((slot, i) => {
              const angle = (i / targetSize) * 360;
              return (
                <ConnectionLine key={`line-${i}`} slot={slot} angle={angle} />
              );
            })}
            ...
            {/* Patient name below circle */}
            <text
              x={CENTER} y={CENTER + PATIENT_R + 15}
              textAnchor="middle"
              fontFamily="system-ui, sans-serif"
              fontSize={12}
              fill={C.muted}
            ...
            <g>
              <rect
                x={CENTER - 14} y={CENTER + PATIENT_R + 22}
                width={28} height={17}
                rx={8}
                fill={C.redBg}
                stroke={C.red}
              ...
              />
              <text
                x={CENTER} y={CENTER + PATIENT_R + 33}
                textAnchor="middle"
                fontFamily="system-ui, sans-serif"
                fontSize={11}
                fontWeight={700}
            ...
            {/* "8 donors needed" */}
            <text
              x={CENTER} y={CENTER + PATIENT_R + 50}
              textAnchor="middle"
              fontFamily="system-ui, sans-serif"
              fontSize={11}
              fill={C.muted}
            >
              {targetSize} donors needed
            </text>
              ...
              return (
                <g key={`donor-${i}`}>
                  ...
                  <DonorNode
                    ...
                    selected={slot && slot.id === selectedId}
                  />
                </g>
              );
            })}
          ...
          {/* ── Popover (absolutely over SVG container) ── */}
          {selectedDonor && (
            <Popover
              donor={selectedDonor}
              svgRef={svgRef}
        ...
        <div style={{
          ...
          {confirmedCount > 0 && (
            <span>
              <span style={{ color: C.green }}>●</span> {confirmedCount} Confirmed
            </span>
          )}
          {scheduledCount > 0 && (
            <span>
              <span style={{ color: C.green, opacity: 0.6 }}>●</span> {scheduledCount} Scheduled
            </span>
          )}
          {awaitingCount > 0 && (
            <span>
              <span style={{ color: C.amber }}>●</span> {awaitingCount} Awaiting
            </span>
          )}
          {restingCount > 0 && (
            <span>
              <span style={{ color: C.gray }}>●</span> {restingCount} Resting
            </span>
          )}
          {lapsedCount > 0 && (
            <span>
              <span style={{ color: C.redErr }}>●</span> {lapsedCount} Lapsed
            </span>
          )}
          {openCount > 0 && (
            <span>
              <span style={{ color: C.gray }}>●</span> {openCount} Open
            </span>
          )}
        </div>
        ...
        <div style={{
          ...
          fontWeight: 500,
          ...(allFilled
            ? { background: C.greenBg, color: '#065f46' }
            : hasLapsed
            ? { background: C.amberBg, color: '#92400e' }
            : { background: '#f9fafb', color: C.muted }),
        }}>
          {allFilled && (
            <span>Bridge is complete ✓</span>
          )}
          {!allFilled && hasLapsed && (
            <>
              <span>
                Bridge needs attention — {lapsedCount} donor{lapsedCount > 1 ? 's' : ''} lapsed
              </span>
              <button
                onClick={() => onHeal && onHeal()}
                style={{
                  marginLeft: 12,
                  fontSize: 12,
                  fontWeight: 600,
